    ...
    if action == 'update' and tribe_form.is_valid():
        ...
        if is_member:
            ...
            request.user.message_set.create(
                message=_("You have joined the team %(tribe_name)s") % {"tribe_name": tribe.name})
            if notification:
            ...
            request.user.message_set.create(
                message=_("You have already joined team %(tribe_name)s") % {"tribe_name": tribe.name})
    elif action == 'leave':
        tribe.members.remove(request.user)
        request.user.message_set.create(message="You have left the team %(tribe_name)s" % {"tribe_name": tribe.name})
        if notification:
